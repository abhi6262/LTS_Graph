...
def FindCommonMsgSegments(message_group):
    ...
    for it in KeyCombinations:
        ...
        if common:
            ...
            for com in common:
                if com in CommonMsgSegs.keys():
                    CommonMsgSegs[com].extend([it[0]])
                    CommonMsgSegs[com].extend([it[1]])
                else:
                    CommonMsgSegs[com] = [it[0]]
                    CommonMsgSegs[com].extend([it[1]])
            #CommonMsgSegs.extend(common)
        else:
            print "No common message segments found in between Message Group: ", it[0], it[1], "\n"

    for key in CommonMsgSegs.keys():
        CommonMsgSegs[key] = list(set(CommonMsgSegs[key]))
...
def NewGrMessageSel(Mtrace, Bufrem, epsilon, Gninc, Mrem, Mcomm, TotalBits, message_width, sysnodes, listmsg, countlistmsg, x, x_y, Infocurr, Ginc):
    ...
    for msg in Mrem:
        ...
        if Bufrem > epsilon:
           if msg in Gninc:
               try:
                   Mpcomm = [item for item in Mcomm.keys() if item in message_group[msg].keys()]
               except KeyError:
                   pass
	       if Mpcomm:
	           Mtilde = []
